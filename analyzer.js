...
async function analyzeDeal() {
  ...
  const localResult = localCalc(purchase, rehab, closing, holding, agent, rent, comps, [...SELECTED_EXITS]);
  const calc = localResult;

  // ── Step 2: Build GPT-4o prompt ──
  const C = comps;
  const compsText = C.length
    ? C.map((c,i) => `Comp ${i+1}: ${c.address||'N/A'} — $${(c.sold_price||0).toLocaleString()} | ${c.sqft||'?'} sqft | ${c.beds||'?'} beds`).join('\n')
    : 'No se proporcionaron comparables — usa tu conocimiento del mercado de Georgia.';

  const a0 = calc.analysis || {};
  const s0 = calc.strategies || {};
  const ff0 = s0.fix_flip||{}, ltr0 = s0.rental||{}, br0 = s0.brrrr||{}, ws0 = s0.wholesale||{}, str0 = s0.airbnb||{};

  const gptPrompt = `Eres un experto en inversiones inmobiliarias en Estados Unidos con 20+ años de experiencia en el mercado de Georgia/Atlanta. Analiza este deal y responde SOLO con JSON válido, sin markdown.

PROPIEDAD: ${addr}, ${city}, GA ${zip}
Precio compra: $${purchase.toLocaleString()} | Rehab: $${rehab.toLocaleString()} | Inversión total: $${(a0.total_investment||0).toLocaleString()}
ARV estimado: $${(a0.arv||0).toLocaleString()} | Closing: $${closing.toLocaleString()} | Holding: $${holding.toLocaleString()}

COMPARABLES:
${compsText}

NÚMEROS CALCULADOS:
Fix&Flip → Ganancia: $${(ff0.profit||0).toLocaleString()} | ROI: ${(ff0.roi||0).toFixed(1)}% | MAO 70%: $${(ff0.mao70||0).toLocaleString()}
Wholesale → MAO: $${(ws0.mao||0).toLocaleString()} | Fee: $${(ws0.assignment_fee||0).toLocaleString()}
Rental LTR → Cashflow: $${(ltr0.monthly_cashflow||0).toLocaleString()}/mes | Cap Rate: ${(ltr0.cap_rate||0).toFixed(1)}% | CoC: ${(ltr0.cash_on_cash||0).toFixed(1)}%
BRRRR → Refi: $${(br0.refinance_amount||0).toLocaleString()} | Cash-out: $${(br0.cash_out||0).toLocaleString()} | Cashflow: $${(br0.cashflow||0).toLocaleString()}/mes
Airbnb STR → Ingreso bruto: $${(str0.gross_monthly||0).toLocaleString()}/mes | Cashflow: $${(str0.cashflow||0).toLocaleString()}/mes | CoC: ${(str0.coc||0).toFixed(1)}%
Score calculado: ${a0.deal_score||0}/100 | Recomendación base: ${a0.recommendation||'N/A'}

Responde con este JSON exacto (sin comas en números):
{
  "deal_score": número 0-100,
  "recommendation": "GOOD DEAL" o "MARGINAL DEAL" o "REJECT",
  "arv": número,
  "best_strategy": "fix_flip" o "wholesale" o "rental" o "brrrr" o "airbnb" o "subject_to" o "seller_finance",
  "total_investment": número,
  "estimated_profit": número,
  "roi": número,
  "executive_summary": "3-4 oraciones directas sobre el deal y si vale la pena",
  "strengths": ["fortaleza 1", "fortaleza 2", "fortaleza 3"],
  "risks": ["riesgo 1", "riesgo 2", "riesgo 3"],
  "red_flags": [],
  "strategy_recommendation": "Párrafo detallado: qué estrategia usar, por qué, cómo ejecutarla, números y plazos específicos",
  "market_insight": "Análisis del mercado local de ${city||'Georgia'}: tendencias, demanda, competencia, zonas de crecimiento",
  "negotiation_tips": ["tip 1", "tip 2", "tip 3"],
  "ai_advisor": "Consejo final del experto en 1-2 oraciones. Qué harías TÚ con este deal.",
  "comps_analysis": "¿El ARV está justificado con los comps?",
  "exit_scores": {"fix_flip":0,"wholesale":0,"rental":0,"brrrr":0,"airbnb":0,"subject_to":0,"seller_finance":0}
}`;

  let result;
  try {
    const gptRaw = await askGPT(
      'Eres un experto en inversiones inmobiliarias en Georgia. Responde SOLO con JSON válido, sin markdown ni texto adicional.',
      gptPrompt,
      2500
    );
    const cleaned = gptRaw.replace(/\`\`\`json/gi,'').replace(/\`\`\`/g,'').trim();
    const ai = JSON.parse(cleaned);

    // Merge AI with local calculations
    result = {
      status: 'ai',
      property_id: null,
      ...
      analysis: {
        ...
        rehab_cost:       rehab,
        total_investment: ai.total_investment || a0.total_investment,
        estimated_profit: ai.estimated_profit || a0.estimated_profit,
        roi:              ai.roi || a0.roi,
        deal_score:       ai.deal_score || a0.deal_score,
        recommendation:   ai.recommendation || a0.recommendation,
        best_strategy:    ai.best_strategy || a0.best_strategy,
        executive_summary:       ai.executive_summary || '',
        strengths:               ai.strengths || [],
        risks:                   ai.risks || [],
        red_flags:               ai.red_flags || [],
        strategy_recommendation: ai.strategy_recommendation || '',
        market_insight:          ai.market_insight || '',
        negotiation_tips:        ai.negotiation_tips || [],
        comps_analysis:          ai.comps_analysis || '',
        exit_scores:             ai.exit_scores || {}
      },
      strategies:  calc.strategies,
      ai_advisor:  ai.ai_advisor || '',
      comps_used:  comps.length
    };
  } catch(e) {
    console.warn('GPT-4o error, usando cálculo local:', e.message);
    result = localResult;
  }
